...
void init_devices( void )
{
    ...
    pinMode( PIN_BRAKE_SERVO_PEDAL_POSITION_SENSOR_HIGH, INPUT );
    pinMode( PIN_BRAKE_SERVO_PEDAL_POSITION_SENSOR_LOW, INPUT );
    pinMode( PIN_BRAKE_SERVO_PEDAL_POSITION_SPOOF_HIGH, INPUT );
    pinMode( PIN_BRAKE_SERVO_PEDAL_POSITION_SPOOF_LOW, INPUT );
...
void init_communication_interfaces( void )
{
    ...
    // Set buffer 0 to filter only brake_servo module and global messages
...
{
    timer1_init( OSCC_BRAKE_REPORT_PUBLISH_FREQ_IN_HZ, publish_brake_servo_report );
}
