...
static void process_brake_servo_command(
...
void publish_brake_servo_report( void )
{
    ...
    brake_report.enabled = (uint8_t) g_brake_servo_control_state.enabled;
    brake_report.operator_override = (uint8_t) g_brake_servo_control_state.operator_override;
    brake_report.dtcs = g_brake_servo_control_state.dtcs;
...
void publish_fault_report( void )
{
    ...
    fault_report.dtcs = g_brake_servo_control_state.dtcs;
    ...
    const can_frame_s * const frame )
{
    if ( frame != NULL )
    {
             ...
             && (frame->data[1] == OSCC_MAGIC_BYTE_1) )
        {
            ...
            {
                process_brake_servo_command( frame->data );
            }
        ...
        const uint8_t * const data )
{
    if ( data != NULL )
    {
        const oscc_brake_command_s * const brake_servo_command =
                (oscc_brake_command_s *) data;

        const float clamped_position = CONSTRAIN(
                brake_servo_command->pedal_command,
                MINIMUM_BRAKE_SERVO_COMMAND,
                MAXIMUM_BRAKE_SERVO_COMMAND);

        float spoof_voltage_low = BRAKE_SERVO_POSITION_TO_VOLTS_LOW( clamped_position );

        spoof_voltage_low = CONSTRAIN(
                spoof_voltage_low,
                BRAKE_SERVO_SPOOF_LOW_SIGNAL_VOLTAGE_MIN,
                BRAKE_SERVO_SPOOF_LOW_SIGNAL_VOLTAGE_MAX);

        float spoof_voltage_high = BRAKE_SERVO_POSITION_TO_VOLTS_HIGH( clamped_position );

        spoof_voltage_high = CONSTRAIN(
                spoof_voltage_high,
                BRAKE_SERVO_SPOOF_HIGH_SIGNAL_VOLTAGE_MIN,
                BRAKE_SERVO_SPOOF_HIGH_SIGNAL_VOLTAGE_MAX);
        ...
        update_brake_servo( spoof_value_high, spoof_value_low );
    }
}
